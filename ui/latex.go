...
// This file renders LaTeX block math ($$…$$ and \[…\]) as images in glow's
// streamed markdown output. glamour has no math support and exposes no extension hook, so
...
func mathSentinel(i int) string {
	return fmt.Sprintf("%cGLOWMATH%d%c", mathSentinelMarker, i, mathSentinelMarker)
}
...
var (
	...
	sentinelRe = regexp.MustCompile(fmt.Sprintf("%cGLOWMATH(\\d+)%c", mathSentinelMarker, mathSentinelMarker))
...
func newMathRenderer(ctx context.Context) *mathRenderer {
	...
	// Transmit images straight to the controlling terminal. glow's streaming TUI
	// renders to stderr and stdout may be redirected, so prefer /dev/tty.
...
// caller waiting for outstanding work — e.g. before quitting at EOF — is never
// left hanging on a fetch that errored.
